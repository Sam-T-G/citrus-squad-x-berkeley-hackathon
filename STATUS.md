...
- **2026-06-21** — Hardened TTS playback and backed out a think-provider experiment. `TTSPlayer` now connects its player node to the mixer in float32 and converts the agent's Int16 chunks on the way in, instead of the direct Int16 mixer connection that can play silent on device; the full device build still passes (`BUILD SUCCEEDED`, no new warnings). Tried again to move the Deepgram think tier to managed Claude (`claude-4-5-haiku-latest`) and reverted to `gpt-4o-mini`, because that model still returns `INVALID_SETTINGS: model not available` on this account (same finding as the earlier entry). Left a note in `VoiceSession.sendSettings` so it is not re-attempted without provisioning, and recorded the only working route to Claude-as-think without managed access (a bring-your-own OpenAI-compatible endpoint, not worth the risk in the closing hours). Rewrote the repo `README.md` to match the built state: the voice and Claude tiers, the on-device CoreML vision path marked built rather than planned, and a known-risks section. Changes are local only, not yet committed. Next: commit, then bench the belt hardware and run the thermal soak before judging.

- **2026-06-21** — Wired the evaluator-optimizer into the describe path and pre-warmed the Claude schemas. `describeScene` now runs a Haiku draft, the on-device `SpokenLineGuard`, then a Sonnet `verify` pass, all inside one voice budget: the verify only uses the time the draft left, so the two-model path never holds the Deepgram turn longer than the single-model path did, and a verify that runs out of time falls back to the guard-approved draft. `ClaudeClient.verify` took a `timeout` parameter, and `prewarm()` now compiles the `VerifiedLine` and `VisionRead` schemas at launch so the first describe and the first `read_sign` do not pay a cold schema compile on stage. Device build passes (`BUILD SUCCEEDED`). Updated `VOICE-AI-PIPELINE.md` so the guard reads as the always-on backstop with the verify layered on top, and added `PITCH-NOTES.md` with the Claude and Deepgram pitch, the tool table, and the claim guardrails. Changes local only. Next: confirm describe and read on device, then commit.
